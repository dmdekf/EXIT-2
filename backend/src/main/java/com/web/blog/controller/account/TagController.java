...
public class TagController {
	...
	@ApiOperation(value = "tag 리스트", response = List.class)
	@GetMapping("/searchTag/{tag}")
	public Object searchTag( @PathVariable String tag) throws Exception {
		List<Taglist> t = tagDao.findByTag(tag);
		ResponseEntity response = null;
		final BasicResponse result = new BasicResponse();
		if(t!=null) {
			result.status = true;
	...
	@ApiOperation(value = "tag 리스트", response = List.class)
	@GetMapping("/searchBoard/{bid}")
	public Object searchBoard( @PathVariable int bid) throws Exception {
		List<Taglist> t = tagDao.findByBid(bid);
		ResponseEntity response = null;
		final BasicResponse result = new BasicResponse();
		if(t!=null) {
			result.status = true;
